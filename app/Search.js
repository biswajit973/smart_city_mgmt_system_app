...
export default function Search() {
    ...
    return ( <
        ...
        <
        Text style = {
            { color: '#fff', fontSize: 10, fontWeight: 'bold' } } > 3 < /Text> <
        /View> <
        /TouchableOpacity> <
        /View> <
        /View> <
        ...
        ScrollView contentContainerStyle = {
            { paddingBottom: 20 } } > { /* Account Modal */ } <
        ThemedText style = {
            [styles.sectionTitle, { color: ORANGE, marginTop: 18 }] } > Services < /ThemedText> {
            filteredServices.length === 0 ? ( <
                ThemedText style = {
                    { textAlign: 'center', marginTop: 30, color: '#888' } } > No services found. < /ThemedText>
            ) : (
                filteredServices.map((item, idx) => ( <
                    View key = { idx }
                    style = {
                        [styles.serviceRow, { backgroundColor: '#fff', borderWidth: 0, borderRadius: 16, shadowColor: ORANGE, shadowOpacity: 0.08, shadowRadius: 8, marginBottom: 18 }] } >
                    <
                    View style = { styles.serviceTextBox } >
                    <
                    ThemedText style = {
                        [styles.serviceTitle, { color: ORANGE }] } > { item.title } < /ThemedText> <
                    ThemedText style = { styles.serviceDesc } > { item.desc } < /ThemedText> <
                    TouchableOpacity style = {
                        { backgroundColor: ORANGE, borderRadius: 16, paddingVertical: 5, paddingHorizontal: 18, alignSelf: 'flex-start', marginTop: 10, shadowColor: ORANGE, shadowOpacity: 0.12, shadowRadius: 6 } }
                    onPress = {
                        () => item.onPress ? item.onPress(router) : null }
                    activeOpacity = { 0.85 } >
                    <
                    ThemedText style = {
                        { color: '#fff', fontWeight: 'bold', fontSize: 12 } } > Book Now < /ThemedText> <
                    /TouchableOpacity> <
                    /View> <
                    View style = {
                        [styles.iconBox, { borderWidth: 0, borderRadius: 16 }] } >
                    <
                    Image source = { item.image }
                    style = {
                        { width: 120, height: 100, resizeMode: 'contain', borderRadius: 16 } }
                    /> <
                    /View> <
                    /View>
                ))
        ...
        /ScrollView> { /* Notification Modal for global notifications - only one instance at the root */ }
